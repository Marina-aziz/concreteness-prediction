...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Parameters for training word embeddings from BERT')
    ...
    parser.add_argument('--pretrained_bert_model', action='store', type=str, default="bert-base-uncased",
                        help='Name of the pretrained BERT model  [default:bert-base-uncased]')
    ...
    parser.add_argument('--word_emb_size', action='store', type=int, default=768,
                        help='word embedding dimensions - must be equal to the embedding dimensions of the loaded BERT model [default:768]')
    ...
    parser.add_argument('--MAX_LEN', action='store', type=int, default=500,
                        help='Maximum length of a sentence after using the BERT tokenizer. Sentences with more than this length are pruned. [default:200]')
    ...
    model = BertModel.from_pretrained(args.pretrained_bert_model)
    model.half()
        ...
        for j, para_buffer_indices in enumerate(para_buffer_dataloader):
            ...
            for k, data in enumerate((para_dataloader)):
                ...
                for para_id, index in enumerate(para_indices):
                    ...
                    paragraphs.append("[CLS] "+ paragraph + " [SEP]")
                ...
                for paragraph in paragraphs:
                    ...
                    if len(tokenized_paragraph) > args.MAX_LEN:
                        ...
                        tokenized_paragraph.append("[SEP]")
